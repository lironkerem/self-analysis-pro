...
const NumerologyPairRules = {
  ...
  // 20
  "2-8-legacy": "a figure of authority whose purpose is harmony. You balance command with care, turning dominance into partnership.", // special phrasing
  // 21
  ...
  // 33-specific pairs (canonical key will be "6-33" etc sorted)
...
// Normalize pair keys: function will sort numbers when checking

// -----------------------------
// Numerology Triplet Rules (including 33 triplets)
// -----------------------------
const NumerologyTriplets = {
  // example triplets (sorted keys)
  ...
  // (other numerology triplets could be added as needed)
};
...
function numerologySpecialOverride(nums) {
  ...
  // other number triple override (generic)
  for (const numStr in counts) {
    const num = Number(numStr);
    if (counts[num] >= 3) {
      // Use the single-number template for emphasis
...
const AstrologyTemplates = {
  ...
  moon: {
    ...
    signs: {
      ...
      taurus: "With Moon in Taurus, your emotions are steady, calm, and rooted in calm.",
  ...
  elements: {
    fire: "a flame that burns with passion, courage and creative drive",
    earth: "a mountain of patience, discipline and grounded strength",
    air: "a breeze of curiosity, vision and quicksilver thought",
    water: "an ocean of depth, feeling and intuitive wisdom"
  }
};

// astrology element mapping (lowercase signs)
const SignToElement = {
...
const AstrologyTriplets = {
  ...
  // add more triads later if desired
};
...
function checkAstrologyTriplet(astrology) {
  const { sun, moon, rising } = astrology;
  if (!sun || !moon || !rising) return null;
  // create a normalized key sorted alphabetically so order doesn't matter
...
// -----------------------------
// Helper utilities for checking pairs/triplets (numerology)
// -----------------------------
...
// Numerology pair check returns array of matching poetic lines (could be multiple)
function checkNumerologyPairs(nums) {
  ...
  for (let i = 0; i < nums.length; i++) {
    for (let j = i + 1; j < nums.length; j++) {
      ...
      // special: legacy key "2-8-legacy" we map if present
      if (key === "2-8" && NumerologyPairRules["2-8-legacy"]) insights.push(NumerologyPairRules["2-8-legacy"]);
    }
  }
  return insights;
}
  ...
  for (let i = 0; i < nums.length; i++) {
    for (let j = i + 1; j < nums.length; j++) {
      for (let k = j + 1; k < nums.length; k++) {
        ...
        if (NumerologyTriplets[key]) { /* duplicated check intentionally allowed */ }
        // also check hardcoded 33 triplets (we already added above)
      }
    }
  }
  return insights;
...
// -----------------------------
// Combined Pair and Triplet check for numerology, astrology, tree interactions
// (allows mixing later across systems if desired)
// -----------------------------
...
function renderNumerologySinglesBlock(numerology) {
  ...
  const parts = [];
  if (NumerologySingles[lifePath]) {
    parts.push(`Your Life Path ${lifePath} shows you are ${NumerologySingles[lifePath].lifePath}`);
  } else if (NumerologySingles[lifePath]) {
    parts.push(NumerologySingles[lifePath].single);
  }

  if (NumerologySingles[destiny]) {
    parts.push(`Your Destiny ${destiny} reveals ${NumerologySingles[destiny].destiny}`);
  }

  if (NumerologySingles[soulUrge]) {
    parts.push(`Deep within, your Soul Urge ${soulUrge} says ${NumerologySingles[soulUrge].soulUrge}`);
  }

  if (NumerologySingles[personality]) {
...
// closing composition: combine three closings into a single elegant sentence
function composeClosings(numerologyClosing, astrologyClosing, treeClosing) {
  // prefer a combined final sentence that touches all three if available
...
// -----------------------------
// MAIN buildNarrative(user)
// -----------------------------
// FIX #7: In your narrativeEngine.js, find this section around line 548-560
...
function buildNarrative(user) {
  ...
  // prepare numerology numbers as integers
  ...
  // 1) Numerology special override (3+ identical)
  const numerologySpecial = numerologySpecialOverride(nums);
  if (numerologySpecial) {
    // Build a compact letter: special numerology override, then also include basic astrology + tree short lines
    ...
    // Add a short astrology mention if present
    if (astrologyOverride) {
      letter += `${astrologyOverride}\n\n`;
    } else {
      if (astrologyTrip) letter += `${astrologyTrip}\n\n`;
      if (astrologyPairs && astrologyPairs.length) letter += astrologyPairs.join(" ") + "\n\n";
      // single astrology lines
  ...
  // 2) Start regular narrative assembly
  let narrative = `${firstName}, my Dear Seeker,\n\n`;

  // 2a: Numerology triplets and pairs insights - FIX: Only add if they exist
  const numerologyTripPairs = gatherNumerologyInsights(nums);
  if (numerologyTripPairs.triplets && numerologyTripPairs.triplets.length) {
    narrative += numerologyTripPairs.triplets.join(" ") + "\n\n"; // Add newlines after
  }
  if (numerologyTripPairs.pairs && numerologyTripPairs.pairs.length) {
  ...
  // 3) Astrology section: overrides, triplets, pairs, singles, closing
  const astrology = user.astrology || {};
  ...
  if (astroOverride) {
    ...
    if (astroPairs && astroPairs.length) narrative += astroPairs.join(" ") + " ";
    // singles
    narrative += renderAstrologySinglesBlock(astrology);
    // closing
    const astroClosing = getAstrologyClosing(astrology);
    narrative += astroClosing + "\n\n";
  }
  ...
  // 5) Cross-system pairs & triplets (optional): combine numerology numbers with astrology element tendencies
  // Compute numerology dominant archetype
  ...
  // Astrology element dominance
  const astro = user.astrology || {};
  const sun = sanitizeSign(astro.sun), moon = sanitizeSign(astro.moon), rising = sanitizeSign(astro.rising);
  ...
  // gentle cross-linking sentence if meaningful
...
function getNumerologySummary(numerology) {
  ...
  if (lifePath && NumerologySingles[lifePath]) {
    lines.push(`Life Path ${lifePath}: ${NumerologySingles[lifePath].lifePath}`);
  }
  if (destiny && NumerologySingles[destiny]) {
    lines.push(`Destiny ${destiny}: ${NumerologySingles[destiny].destiny}`);
  }
  if (soulUrge && NumerologySingles[soulUrge]) {
    lines.push(`Soul Urge ${soulUrge}: ${NumerologySingles[soulUrge].soulUrge}`);
  }
  if (personality && NumerologySingles[personality]) {
    lines.push(`Personality ${personality}: ${NumerologySingles[personality].personality}`);
  }
  return lines;
}
